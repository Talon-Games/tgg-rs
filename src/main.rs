...
pub fn main() {
    ...
    let tgg_file = match TggFile::custom_crossword(
        "Test Crossword",
    ...
    let path_main = Path::new("./crossword.tgg");
    let path = Path::new("./tests/crosswords/crossword.tgg");

    match std::fs::write(path_main, tgg_file.to_bytes()) {
        Ok(_) => println!("saved main"),
        Err(err) => {
            eprintln!("{}", err);
            std::process::exit(1);
        }
    ...
    match std::fs::write(path, tgg_file.to_bytes()) {
        Ok(_) => println!("saved test"),
        Err(err) => {
            eprintln!("{}", err);
            std::process::exit(1);
        }
    ...
    let tgg_file = match TggFile::from_bytes(bytes) {
        Ok(tgg_file) => tgg_file,
        Err(err) => {
            eprintln!("{}", err);
            std::process::exit(1);
